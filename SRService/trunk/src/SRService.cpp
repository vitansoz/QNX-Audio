...
#define PPS_FILE_SR "/pps/hinge-tech/asr/control"
...
NaviControl g_naviControl;
MultimediaControl g_multimediaControl;
...
bool procMsgResult(const char *result, int len)
{
   ...
   if (doc)
   {
	 ...
	 g_player->Start("车灯操作已完成"/*voice.c_str()*/);
      ...
      //}
      else if (g_naviControl.match((char*)rawtext))
      {
         voice = "您说的是，";
	 if (rawtext)
	 {
	 ...
	 voice += ",操作已完成";
	 g_player->Start("导航操作已完成"/*voice.c_str()*/);
      }
      else if (g_multimediaControl.match((char*)rawtext))
      {
         voice = "您说的是，";
	 if (rawtext)
	 {
	 ...
	 voice += ",操作已完成";
	 g_player->Start("多媒体操作已完成"/*voice.c_str()*/);
      }
      else
      {
	 if (content)
...
void* NaviFeedbackFunc(void*)
{
  ...
  fd = open("/pps/services/geolocation/vr_status", O_RDWR, 0666);

  if (fd > 0) {
    std::cout << "/pps/services/geolocation/vr_status open OK" << std::endl;
  } else {
    std::cout << "/pps/services/geolocation/vr_status open error:" << strerror(errno) << std::endl;
    return 0;
  }
  ...
  while ( 1 ) {
    ...
    if ( FD_ISSET ( fd, &rfds ) ) {
      ...
      length = read ( fd, pps_buf, sizeof ( pps_buf ) - 1 );
      printf ( "***length:%d\n%s\n", length, pps_buf );
      ...
      err = pps_decoder_get_int ( &decoder, "feedback", &fbk );
      if ( !err ) {
        switch (fbk) {
          case 1:
            g_player->Start("您未设置导航路线");
            break;
          case 2:
            g_player->Start("没有搜索到目的地");
            break;
          case 3:
            g_player->Start("您家的地址未设置");
            break;
          case 4:
            g_player->Start("您公司的地址未设置");
            break;
          case 5:
            g_player->Start("没有找到该城市的路况信息");
            break;
          default:
            break;
        }
...
int main(int argc, char *argv[]) 
{
   ...
   ret = pthread_create ( &readpthr, NULL, &NaviFeedbackFunc, NULL );
