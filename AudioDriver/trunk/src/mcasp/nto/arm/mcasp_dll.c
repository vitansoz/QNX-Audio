...
static int 
i2c_write(int fd,  uint8_t* data, int data_num, uint8_t slave_addr)
{
	  int err = 0 ;
#define MAX_PACKET_SIZE 200
	  ...
	  {
    		 ado_error("Failed to write to codec \n" );
      }
...
static void mute_channel(HW_CONTEXT_T * mcasp_card)
{
	//mute when stop playback 
    uint8_t buf[5] = {0};
			
	buf[0] = (MUTE_ADDR & 0xFF0000) >> 16;
	buf[1] = (MUTE_ADDR & 0xFF00) >> 8;
	buf[2] = MUTE_ADDR & 0xFF;
	buf[3] = 0x00;
	buf[4] = 0x00;
   	i2c_write(mcasp_card->i2c_dev, buf, 5, mcasp_card->codec_i2c_addr);
    return;
}
static void unmute_channel(HW_CONTEXT_T * mcasp_card)
{
	uint8_t buf[5] = {0};
...
int32_t mcasp_trigger(HW_CONTEXT_T * mcasp_card, PCM_SUBCHN_CONTEXT_T * pc, uint32_t cmd)
{
	...
	if (cmd == ADO_PCM_TRIGGER_GO)
	{
		...
		if (pc == &mcasp_card->play_strm.subchn)
		{
			...
			while (((mcasp_card->mcasp->GBLCTL & XGBLCTL_XSRCLR) != XGBLCTL_XSRCLR) && timeout--)
				nanospin_ns(100);
			if (timeout <= 0)
				ado_error("%s %s %d :tx serializer not enabled", __FILE__, __func__, __LINE__);
			...
			while (((mcasp_card->mcasp->WFIFOSTS & WFIFOSTS_WLVL) < 63) && timeout--)
				nanospin_ns(100);
			if (timeout <= 0)
				ado_error("%s %s %d :AFIFO is not full, %d", __FILE__, __func__, __LINE__, mcasp_card->mcasp->WFIFOSTS & WFIFOSTS_WLVL);
			...
			while (((mcasp_card->mcasp->XSTAT & XSTAT_XDATA)) && timeout--)
				nanospin_ns(100);
			if (timeout <= 0)
				ado_error("%s %s %d :TX FIFO is never filled by EDMA", __FILE__, __func__, __LINE__);
			/* Clear underrun flag */
			...
			while (((mcasp_card->mcasp->GBLCTL & XGBLCTL_XSMRST) != XGBLCTL_XSMRST) && timeout--)
				nanospin_ns(100);
			if (timeout <= 0)
				ado_error("%s %s %d :unable to release TX State Machine from reset", __FILE__, __func__, __LINE__);
#ifdef STREAM_CONTROL
			delay(100);
			ado_error("%s %s %d :unmute before playback", __FILE__, __func__, __LINE__);
		//	ado_mutex_lock(&mcasp_card->cmd_lock);
			mcasp_card->cmd = 1;
   			unmute_channel(mcasp_card);
		//	ado_mutex_unlock(&mcasp_card->cmd_lock);
#endif		
		...
		else
		{
			if (mcasp_card->cap_aif.active == 0)
			{
				...
				while (((mcasp_card->mcasp->GBLCTL & RGBLCTL_RSRCLR) != RGBLCTL_RSRCLR) && timeout--)
					nanospin_ns(100);
				if (timeout <= 0)
				    ado_error("%s %s %d :rx serializer not enabled", __FILE__, __func__, __LINE__);
				...
				while (((mcasp_card->mcasp->GBLCTL & RGBLCTL_RSMRST) != RGBLCTL_RSMRST) && timeout--)
					nanospin_ns(100);
				if (timeout <= 0)
					ado_error("%s %s %d :unable to release RX State Machine from reset", __FILE__, __func__, __LINE__);
			}
	...
	else
	{
		...
		//	ado_mutex_lock(&mcasp_card->cmd_lock);
			mcasp_card->cmd = 0;
			mute_channel(mcasp_card);
		//	ado_mutex_unlock(&mcasp_card->cmd_lock);	
			delay(100);//100 ms delay
			ado_error("%s %s %d :mute after finish playback", __FILE__, __func__, __LINE__);
#endif
			...
			if (timeout <= 0)
				ado_error("%s %s %d :TXBUF was not emptied", __FILE__, __func__, __LINE__);
...
{
	ado_error("%s %s %d :RX Overrun occured!", __FILE__, __func__, __LINE__);
	mcasp_card->mcasp->RSTAT |= RSTAT_ROVRN;
}
...
#else
	ado_error("%s %s %d : TX Underrun occured! - DMA Position = %d/%d", __FILE__, __func__, __LINE__, nbytes, ado_pcm_dma_int_size(mcasp_card->play_strm.subchn.pcm_config));
#endif
	mcasp_card->mcasp->XSTAT |= XSTAT_XUNDRN;
}
...
subchn_dmacapture(HW_CONTEXT_T *mcasp_card, uint8_t *srcDMAAddr, uint32_t size)
{
	...
	{
		ado_error("%s %s %d : subchn_dmacapture invalid data or size", __FILE__, __func__, __LINE__);
		return;
	}
...
int ctrl_init(HW_CONTEXT_T ** hw_context, ado_card_t * card, char *args)
{
 ...
 //   ado_mutex_init(&mcasp_card->cmd_lock);
	/* Initialize the card */
